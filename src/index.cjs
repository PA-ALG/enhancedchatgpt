...
// node_modules/.pnpm/registry.npmmirror.com+tsup@6.7.0_typescript@4.9.5/node_modules/tsup/assets/cjs_shims.js
...
// ../node_modules/.pnpm/axios@1.3.4/node_modules/axios/dist/node/axios.cjs
var require_axios = __commonJS({
  "../node_modules/.pnpm/axios@1.3.4/node_modules/axios/dist/node/axios.cjs"(exports, module2) {
    "use strict";
    init_cjs_shims();
...
Current date: ${currentDate}`;
  ...
  async sendMessage(text, opts = {}) {
    ...
    const updatedMessages = await this._updateMessage(
      messages,
      maxTokens,
      numTokens,
      opts
    ...
    const newMaxTokens = updatedMessages.newMaxTokens;
    const result = {
      role: "assistant",
      id: (0, import_uuid.v4)(),
...
${message.content}`]);
  ...
  async _updateMessage(oldMessage, maxTokens, numTokens, opt) {
    try {
      const postUrl = opt.enhancedPromptUrl;
      const userLabel = USER_LABEL_DEFAULT;
      const assistantLabel = ASSISTANT_LABEL_DEFAULT;
      const maxNumTokens = this._maxModelTokens - this._maxResponseTokens;
      let newMessage = [...oldMessage];
      let lastObject = newMessage[newMessage.length - 1];
      const now = /* @__PURE__ */ new Date();
      const year = now.getUTCFullYear();
      const month = now.getUTCMonth() + 1;
      const date = now.getUTCDate();
      const hour = now.getUTCHours();
      const ts = `${year}-${month}-${date}-${hour}`;
      const auth = `Bearer pafc-${ts}`;
      const response = await axios.post(postUrl, {
        query: lastObject.content,
        ts
      }, {
        headers: {
          "Content-Type": "application/json",
      ...
      lastObject.content = response.data;
      const prompt = newMessage.reduce((prompt2, message) => {
        switch (message.role) {
          case "system":
            return prompt2.concat([`Instructions:
  ${message.content}`]);
  ...
  ${message.content}`]);
      ...
      const isValidPrompt = nextNumTokensEstimate <= maxNumTokens;
      if (prompt && !isValidPrompt) {
        return { oldMessage, maxTokens, numTokens };
      }
      const newNumTokens = nextNumTokensEstimate;
      const newMaxTokens = Math.max(
        1,
        Math.min(this._maxModelTokens - newNumTokens, this._maxResponseTokens)
      );
      return { newMessage, newMaxTokens, newNumTokens };
    } catch (error) {
      console.error(error);
    }
  }
